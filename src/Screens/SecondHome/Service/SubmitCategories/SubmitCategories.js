...
import {Medium, SemiBold, seminBoldLegacy} from '../../../../Colors/Fonfamily';
const SubmitCategories = props => {
  ...
  const getAddresses = async () => {
    try {
      ...
      const apiCall = await axios.get(
        `https://lamv-backend.herokuapp.com/addresses/get-my-address/${userId}`,
      );
  ...
  const uploadOrderImages = async () => {
    try {
      ...
      const apiCall = await axios.post(
        `${baseUrl2}/orders/upload-order-images`,
        formData,
      );
      setLoading(false);
      if (apiCall.status == 200) {
  ...
  const createAddress = async () => {
    try {
      ...
      const userId = await getUserId();
      const apiCall = await axios.post(
        `https://lamv-backend.herokuapp.com/addresses/create-address`,
        {
          info: info,
          phone: phone,
          address: fullAddress,
  ...
  const submitservice = async images => {
    try {
      ...
      console.log(body);
      const submitapi = await axios.post(
        `https://lamv-backend.herokuapp.com/orders/place-order`,
        body,
      );
      ...
      if (submitapi?.status == 200) {
        ...
        Alert.alert(
          ...
          JSON.stringify(loginApiCall.data.info.message),
        );
      }
      setLoading(false);
    } catch (error) {
    ...
    <>
      ...
      <View style={{flex: 1}}>
        <ScrollView
          ...
          {isOffer ? (
            <View style={{padding: 10, width: '95%', alignSelf: 'center'}}>
              <Text
                style={{fontSize: 17, paddingBottom: 10, fontFamily: Medium}}>
                Selected offer:{' '}
              </Text>
              <View
                style={{
            ...
            <View style={{margin: 10}}>
              <Text
                style={{fontFamily: Medium, margin: 5, top: 5, fontSize: 18}}>
                Selected Services
              </Text>
              <View
                style={{
          ...
          <View>
            <Input
              ...
              descBox={true}
              style={{fontSize: 16, fontFamily: Medium, color: '#000'}}
              value={decription}
          ...
          <View>
            <Text
              style={{fontFamily: Medium, margin: 10, top: 5, fontSize: 18}}>
              Add Images
            </Text>
            <View
              style={{
          ...
          <View>
            ...
            {selectedAddress ? (
              ...
              <TouchableOpacity
                ...
                <Text
                  style={{color: priamry, fontFamily: SemiBold, fontSize: 16}}>
                  {allAddresses && !isLoading
                    ? 'Select Address'
                    : 'Add Address'}
                </Text>
              </TouchableOpacity>
          ...
          <View>
            ...
            <View
              ...
              <Pressable
                onPress={() => setSelectedContect('phone')}
                style={{
                  margin: 10,
                  flexDirection: 'row',
                  alignItems: 'center',
                  ...
                  borderRadius: 5,
                  backgroundColor:
                    selectedContect == 'phone' ? priamry : 'white',
                  borderColor: priamry,
                }}>
                <Icon
                  type="FontAwesome"
                  name="phone"
                  color={selectedContect == 'phone' ? 'white' : priamry}
                  size={20}
                />
                <Text
                  style={{
              ...
              <Pressable
                onPress={() => setSelectedContect('email')}
                style={{
                  margin: 10,
                  flexDirection: 'row',
                  alignItems: 'center',
                  ...
                  borderColor: priamry,
                  backgroundColor:
                    selectedContect == 'email' ? priamry : 'white',
                }}>
                <Icon
                  type="MaterialCommunityIcons"
                  name="email"
                  color={selectedContect == 'email' ? 'white' : priamry}
                  size={20}
                />
                <Text
                  style={{
            ...
            <View
              ...
              <Pressable
                ...
                style={{
                  ...
                  justifyContent: 'center',
                  alignItems: 'center',
                  borderColor: isUrgent ? priamry : 'grey',
                  borderRadius: 25 / 2,
                  backgroundColor: isUrgent ? priamry : 'transparent',
                }}>
                {isUrgent ? (
                  <Icon
                    name="check"
                    type="Entypo"
                    color={isUrgent ? 'white' : 'grey'}
                    size={13}
                  />
                ) : null}
              </Pressable>
              <Text
                style={{fontFamily: SemiBold, color: 'grey', paddingLeft: 5}}>
                Choose urgent option, if you would like to hire us for urgent,
                this option may charge you more than usual
              </Text>
            </View>
          </View>
          <ThinLineSeparator margin={15} />
        ...
        <TouchableOpacity
          ...
          <Text style={{color: 'white', fontSize: 18, fontFamily: SemiBold}}>
            Submit
          </Text>
        </TouchableOpacity>
      </View>
        ...
        {allAddresses && !isLoading && !addMode ? (
          ...
          <View>
            <View style={{width: '95%', alignSelf: 'center'}}>
              <Text style={{fontSize: 19, fontFamily: SemiBold, margin: 10}}>
            ...
            <View
              ...
              <Pressable
                ...
                />
                <Text
                  style={{color: priamry, fontSize: 20, fontFamily: Medium}}>
                  Home
                </Text>
              </Pressable>
              <Pressable
            ...
            <View style={styles.mapinputCon}>
              <TextInput
                ...
                value={fullAddress}
                onChangeText={street => setFullAddress(street)}
              />
            </View>
            <View style={styles.mapinputCon}>
              <TextInput
            ...
            <View style={styles.mapinputCon}>
              <TextInput
                placeholderTextColor={'#222'}
                placeholder="Optional Note for rider"
                style={styles.mapinput}
                value={info}
