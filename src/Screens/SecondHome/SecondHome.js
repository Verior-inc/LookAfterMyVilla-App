...
import { View, Text, TouchableOpacity, Image, ImageBackground, FlatList, Pressable, Alert } from 'react-native'
...
const SecondHome = ({ navigation }) => {
    ...
    const offerapicall = async () => {
        ...
        try {
            ...
            showToaster(error?.response?.data?.message ? error?.response?.data?.message : 'Something went wrong, please try again-1')
        }
    }
    const mainservicesapicall = async () => {
        ...
        setUserName(name)
        try {
            const mainserviceapi = await axios.get(`${baseUrl2}/categories/get-maincategories`);
            setData(mainserviceapi.data.data)
            setLoading(false);
                
        } catch (error) {
            setLoading(false);
            showToaster(error?.response?.data?.message ? error?.response?.data?.message : 'Something went wrong, please try again-2')
            ...
            <View style={styles.offerCon}>
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <View style={styles.card}>
                            <ImageBackground source={{uri:`${baseUrl2}/${item?.image}`}}
                                key={index}
                                style={styles.cardBox}
                                imageStyle={{ borderRadius: 10, opacity: 0.7 }}
                            >
                                <View style={{ backgroundColor: "rgba(0,0,0,0.5)", flex: 1, borderRadius: 10 ,padding:15}}>
                                 ...
                                 style={{
                                     ...
                                     <Text style={{color:'white',fontSize:15,fontWeight:'bold'}}>Order</Text>
                                 </TouchableOpacity>
                                </View>
                ...
                <View style={{flex:1}}>
                    ...
                    <FlatList
                        ...
                        renderItem={({ item, index }) => {
                            return <Pressable key={item.id} style={{ flex: 1,margin:10,width:'45%'}} onPress={() => navigation.navigate("SubCategory", { mainservicesid: item._id, mainservicesName: item.name, getuserID: userid })}  >
