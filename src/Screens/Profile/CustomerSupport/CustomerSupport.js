...
const CustomerSupport = props => {
  ...
  const SCREEN_TITLES = {
    Support:
      'Customers safety is our first priority, we will be happy to help you out, enter the query title and description and open the ticket.',
    ReportBug:
      'Report a bug when ever you catch it, because that can help us improve our platform.',
  };
  ...
  const submitCustomerSupport = async () => {
    try {
      ...
      console.log(userID);
      setLoading(true);
      const submitsupportapi = await axios.post(
        `${baseUrl2}/customer-support/create-customer-support`,
        {
          userId: userID,
          title: title,
          description: description,
      ...
      if (submitsupportapi.data.success == true) {
        showToaster(
          'Your ticket has been created successfully, we will contact you soon :)',
          'success',
        );
        props.navigation.navigate('Profile');
      }
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <View style={{padding: 10}}>
        <View style={{flexDirection: 'row', alignItems: 'center'}}>
          <Icon
            type="MaterialIcons"
            name={CustomerSupport ? 'support-agent' : 'bug-report'}
            color={priamry}
            size={30}
          />
          <Text style={{fontSize: 25, fontFamily: SemiBold, paddingLeft: 5}}>
            {CustomerSupport ? 'Customer Support' : 'Report a bug'}
          </Text>
        </View>
        <Text style={{fontSize: 12, fontFamily: Regular, color: 'grey'}}>
          {SCREEN_TITLES[CustomerSupport ? 'Support' : 'ReportBug']}
        </Text>
      </View>
      <View style={{width: '95%', alignSelf: 'center'}}>
        <Input
          borderColor={'grey'}
          borderRadius={3}
          style={{fontFamily: Medium, color: 'grey', fontSize: 16}}
          icon={
            <Icon
              type="MaterialIcons"
              name="title"
        ...
        <Input
          ...
          placeholder={
            CustomerSupport
              ...
              : 'Describe the bug which you catched here, write all the scenarios'
          }
        />
      </View>
      <TouchableOpacity
        style={styles.btn}
        onPress={() =>
          CustomerSupport ? submitCustomerSupport() : ReportBugg()
        }>
        <Text style={styles.btntxt}>
          {CustomerSupport ? 'Open Ticket' : 'Report'}
        </Text>
      </TouchableOpacity>
    </View>
  );
