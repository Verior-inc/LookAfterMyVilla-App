...
const OrderDetail = (props) => {
    ...
    return (
        <>
         ...
         <View style={{width:'100%',paddingLeft:6,height:75,backgroundColor:priamry,flexDirection:'row',alignItems:'center'}}>
             <TouchableOpacity>
                 <MaterialCommunityIcons name='keyboard-backspace' color='white' size={25}/>
             </TouchableOpacity>
             <Text style={{color:'white',fontSize:19,paddingLeft:10,fontWeight:'bold'}}>Order #{items?.orderId}</Text>

            </View>
            <ScrollView contentContainerStyle={{flexGrow:1}}> 
            {
                items?.isBudgetAttached && items?.status_code == 'BUDGET_APPROVAL_PENDING' ? 
                <View style={{width:'95%',borderWidth:1,borderColor:'powderblue',backgroundColor:'powderblue',alignSelf:'center',margin:10,padding:10}}>
             <Text style={{fontWeight:'300'}}>We have attached one budget pdf file with details open and review it, if you agree on the terms accept the offer to proceed with order.</Text>
             <TouchableOpacity style={{alignSelf:'flex-end'}}>
                 <Text style={{fontWeight:'bold',color:priamry,fontSize:18}}>Open</Text>
             </TouchableOpacity>
            </View>
            :
            null
            ...
            <View style={{flexDirection:'row',justifyContent:'space-between',alignItems:'center',paddingHorizontal:10,marginTop:20}}>
                <View>
                    <Text style={{fontWeight:'bold',fontSize:20}}>#{items?.orderId}</Text>
                    <Text style={{color:'grey'}}>{moment(items?.ordered_on).format('DD MMMM YYYY hh:ss A')}</Text>
                </View>
                <Text style={{fontSize:16,fontWeight:'bold'}}>{items?.order_status}</Text>
            </View>
            <Separator/>
            <View style={{width:'95%',alignSelf:'center'}}>  
                <Text style={{fontSize:19,fontWeight:'bold'}}>Customer details</Text>
                <Text>{items?.OrderBy?.name}</Text>
                <Text>{items?.OrderBy?.email}</Text>
                <Text>{items?.OrderBy?.phone}</Text>
                <AddressComponent
        ...
        <View style={{width:'95%',alignSelf:'center'}}>
                ...
                <Text style={{fontSize:19,fontWeight:'bold'}}>Order details</Text>
              {
                  ...
                  >
                  <Text style={{fontSize:15,fontWeight:'bold',color:priamry}}>Budget File?</Text>
                  </TouchableOpacity>
                  :
                  null
              }
                </View>
                <Text style={{fontWeight:'bold'}}>Description: <Text style={{fontWeight:'normal'}}>{items?.description}</Text></Text>  
               <View style={{flexDirection:'row',alignItems:'center',marginTop:10}}>
               
                {
                    ...
                    <View style={{
                       ...
                       <Text style={{fontSize:20,color:'white'}}>{items?.orderedOffer?.description}</Text>
                        <Text style={{fontSize:30,fontWeight:'bold',color:'white'}}>{items?.orderedOffer?.percent_off}%</Text>
                       </View>
                        </ImageBackground>
                    ...
                    <Text>{items?.mainCategory?.name}</Text>
                </View>
                <View>
                    {
                ...
                <FlatList
                    ...
                    renderItem={({ item, index }) => (
                        <View style={styles.card}>
                            <ImageBackground source={{uri:`${baseUrl2}/${item}`}}
                                key={index}
...

          {
              ...
              style={{width:'95%',alignSelf:'center',height:50,backgroundColor:'red',justifyContent:'center',alignItems:'center',borderRadius:5,position:'absolute',bottom:10}}>
                  <Text style={{fontSize:19,fontWeight:'bold',color:'white'}}>Cancel Order</Text>
              </TouchableOpacity>
              :
              null
          }
            ...
            adjustToContentHeight
                    >
                        <View style={{padding:10,flex:1}}>
                            ...
                            <Text style={{fontSize:20,fontWeight:'bold'}}>Cancellation Reason.</Text>
                            <Text style={{fontSize:15,fontWeight:'300'}}>We are so sad that you are cancelling this order, enter your reason can help us improve our service.</Text>
                            </View>
                        <View style={styles.mapinputCon}>
                            <TextInput multiline placeholderTextColor={'black'} placeholder="Describe cancellation reason here" value={reason} onChangeText={(reason) => setReason(reason)} style={styles.mapinput} />
                        </View>
