...
import {Bold, Regular} from '../../../Colors/Fonfamily';
    ...
    try {
      if (!email) {
        ...
        const apiCall = await axios.post(
          `${baseUrl2}/forgot-password/generatecode`,
          {
            email: email,
          },
        );
        ...
        if (apiCall.data.success == true) {
          Alert.alert(JSON.stringify(apiCall.data.OTP));
          props.navigation.navigate('Verification', {
            ForgotCode: apiCall.data.OTP,
    ...
    <View style={{flex: 1, backgroundColor: 'white'}}>
      ...
      <View>
        <View
          ...
          <MaterialIcons name="email" color={priamry} size={100} />
        </View>
        <View style={{padding: 10, marginTop: 10}}>
          <Text style={{fontSize: 25, fontFamily: Bold, textAlign: 'center'}}>
            Email Verification
          </Text>
          <Text
            style={{
            ...
            Enter your registered email address that we can send an otp code to
            that email.
          </Text>
        </View>
      </View>
      <View
        ...
        <Input
          ...
          value={email}
        />
        <TouchableOpacity onPress={Forgot} style={styles.btn}>
          <Text style={styles.btntxt}>Submit</Text>
        </TouchableOpacity>
      </View>
    </View>
