...
const SignUp = props => {
  ...
  const Signup = async () => {
    try {
      ...
      if (!name) {
        showToaster('Please enter the Name');
        return;
      }
      if (!contact) {
        showToaster('Please enter the Phone no');
      } else {
        setLoading(true);
        const checkemail = await axios.post(
          `${baseUrl2}/user/check-verification`,
          {
            email: email,
        ...
        if (checkemail?.data.success == true) {
          Alert.alert(JSON.stringify(checkemail.data.code));
          props.navigation.navigate('Verification', {
            email: email,
            password: password,
            contact: contact,
            name: name,
            verfiedCode: checkemail.data.code,
        ...
        <View sty={styles.mainContainer}>
          ...
          <View style={{padding: 20, marginVertical: '5%'}}>
            ...
            <Text style={{fontFamily: Regular, color: 'grey', fontSize: 13}}>
              Register and hire us for multiple services on finger tips{' '}
            </Text>
